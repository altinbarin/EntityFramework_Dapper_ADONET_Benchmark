...
namespace BusinessLayer
{
    public class ProductManager
    {
        ...
        public List<Product1> GetProductsUsingDapper()
        {
            using (var connection = new SqlConnection(connectionString))
            {
                connection.Open();
                return connection.Query<Product1>("SELECT * FROM dbo.Products").ToList();
            }
        }

        public Product1 GetProductByIdUsingDapper(int id)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                connection.Open();
                return connection.Query<Product1>("SELECT * FROM dbo.Products WHERE Id = @Id", new { Id = id }).FirstOrDefault();
            }
        }

        public int AddDapper(Product1 p)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                connection.Open();
        ...
        public int UpdateDapper(Product1 p)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                connection.Open();
