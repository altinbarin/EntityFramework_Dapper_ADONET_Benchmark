...
namespace HizTesti
{
    public partial class Form1 : Form
    {
        ...
        private void btnDapper_Click(object sender, EventArgs e)
        {
            ...
            long startmemory = GC.GetTotalMemory(true);
            stopwatch.Start();
            list = productManager.GetProductsUsingDapper();
            stopwatch.Stop();
            long endmemory = GC.GetTotalMemory(true);
        ...
        private void DapperIleEkle(object sender, EventArgs e)
        {
            ...
            long startmemory = GC.GetTotalMemory(true);
            stopwatch.Start();
            productManager.AddDapper(new Product1
            {
                ProductName = txtProductName.Text,
                Stock = Convert.ToInt32(txtStock.Text)
            });
        ...
        private void btnGuncelleDapper_Click(object sender, EventArgs e)
        {
            ...
            //var id = dataGridView1.SelectedRows[0].Cells[0].Value;
            var id = Convert.ToInt32(txtId.Text);
            var guncellenecekUrun = productManager.GetProductByIdUsingDapper(id);
